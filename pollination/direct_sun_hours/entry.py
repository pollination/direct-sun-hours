...
# from pollination.alias.outputs.daylight import sort_direct_sun_hours_results
...
class DirectSunHoursEntryPoint(DAG):
    ...
    def create_rad_folder(self, input_model=model):
        ...
        return [
            ...
            {
                'from': CreateRadianceFolder()._outputs.sensor_grids_file,
                'to': 'cumulative-results/grids_info.json'
            },
            {
                'from': CreateRadianceFolder()._outputs.sensor_grids_file,
                'to': 'radiation-results/grids_info.json'
            },
            {
    ...
    results = Outputs.folder(
        source='results',
        # alias=sort_direct_sun_hours_results
    )

    cumulative_results = Outputs.folder(
        source='cumulative_results',
        # alias=sort_direct_sun_hours_results
    )

    radiation_results = Outputs.folder(
        source='radiation_results',
        # alias=sort_direct_sun_hours_results
    )
