...
from .dependencies.direct_sun_hours_ray_tracing import _DirectSunHoursEntryLoopOrchestrator as DirectSunHoursEntryLoopWorkerbee
...
class DirectSunHoursEntryLoopLoop(luigi.Task):
    ...
    def run(self):
        yield [DirectSunHoursEntryLoopWorkerbee(_input_params=self.map_dag_inputs)]
...
class DirectSunHoursEntryLoop(luigi.Task):
    """No description is provided."""
    # global parameters
    _input_params = luigi.DictParameter()
    @property
    ...
    def run(self):
        yield [DirectSunHoursEntryLoopLoop(item=item, _input_params=self._input_params) for item in self.items]
        with open(os.path.join(self.execution_folder, 'direct_sun_hours_raytracing.done'), 'w') as out_file:
            out_file.write('done!\n')
...
class _MainOrchestrator(luigi.WrapperTask):
    ...
    def requires(self):
        return [DirectSunHoursEntryLoop(_input_params=self.input_values), ParseSunUpHours(_input_params=self.input_values)]
